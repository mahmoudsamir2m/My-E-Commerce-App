  ...
  return (
    <>
      <p className="bg-black text-white text-center text-sm py-2">
        Summer Sal For All Swim Suits And Free Express Delivery-OFF 50%!{" "}
      ...
      <nav className="border-b bg-white shadow sticky top-0 z-10">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex-1 w-full">
                ...
                <NavigationMenuList>
                  <NavigationMenuItem>
                    <NavigationMenuLink asChild>
                      <Link
                        href="/"
                        className="hover:bg-white hover:underline hover:text-black focus:bg-white focus:text-black"
                      >
                        Home
                      </Link>
                    </NavigationMenuLink>
            ...
            <div className="hidden md:flex items-center gap-2 ">
              <DropdownMenu>
                <form onSubmit={handleSubmit} className="flex items-center ">
                  ...
                  <button
                    type="submit"
                    className=" text-black px-3 py-1 flex items-center border border-l-0"
                  >
                    <SearchIcon size={24} />
                  </button>
                </form>
          ...
          {/* Mobile Menu */}
          <div
            className={`md:hidden absolute top-full left-0 w-full bg-white border-b shadow-lg z-50 transition-all duration-300 ease-in-out ${
              isMobileMenuOpen
                ? "max-h-105 opacity-100 visible"
          ...
          >
            <div className="p-4 space-y-4">
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-2">
                ...
                <button
                  type="submit"
                  className="bg-black text-white px-3 py-1 rounded flex items-center justify-center gap-1 w-full"
                >
                  <SearchIcon size={16} />
                  Search
                </button>
              ...
              <div className="flex flex-col gap-2 pt-2 border-t">
                <DropdownMenu>
                  <DropdownMenuTrigger className="w-full text-left outline-0 cursor-pointer py-2">
                    {" "}
                    <UserIcon className="inline mr-2" size={20} /> Account
                  </DropdownMenuTrigger>
                ...
                {session.status == "authenticated" && (
                  <>
                    ...
                    <Link
                      href={"/cart"}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="flex items-center gap-2 py-2 text-black hover:underline relative"
                    >
