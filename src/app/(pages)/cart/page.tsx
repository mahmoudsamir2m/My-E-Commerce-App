...
export default function Cart() {
  ...
  if (isLoading || !cartData?.data?.products) {
    return <Loading />;
  }

  // If the cart is empty
  if ((cartData?.numOfCartItems ?? 0) === 0) {
    return (
      <div className="flex flex-col justify-center items-center gap-6">
        <h2 className="text-center text-2xl mt-20">Your Cart Is Empty, </h2>
        <Link
          href={"/products"}
          className="bg-red-500 hover:bg-red-600 text-white py-3 px-6 rounded-md transition-colors"
        >
          Go Shopping
        </Link>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            {cartData?.data?.products?.map((item) => (
              <div
                key={item._id}
                className="grid grid-cols-1 sm:grid-cols-6 items-center gap-4 py-6 border-b border-gray-200"
              >
                {/* Product Info */}
            ...
            <div className="flex justify-between mt-8">
              ...
              <button
                onClick={clearCart}
                className="flex gap-2 bg-red-500 hover:bg-red-600 text-white py-3 px-6 rounded-md transition-colors cursor-pointer"
              >
          ...
          <div className="lg:col-span-1 flex flex-col space-y-8 sticky top-20 self-start">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:space-x-4 mt-8 lg:mt-0">
              ...
              <button
                onClick={handleApply}
                className="bg-red-500 hover:bg-red-600 text-white py-3 px-6 rounded-md transition-colors mt-4 sm:mt-0 cursor-pointer"
              >
                Apply Coupon
              </button>
            </div>
